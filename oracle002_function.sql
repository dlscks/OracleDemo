...
--42523.55을 10의 자리까지 구하시오(반올림)
SELECT round(42523.55, -1)
FROM dual;

--42523.55울 1의 자리까지 구하시오(반올림)
...
-- 날짜 -> 문자
SELECT to_char(sysdate, 'yyyy-mm-dd hh:mi:ss day') --12시간 월요일
FROM dual;
...
--hire_date(입사일)을 이용해서 '홍길동님은 2003년 1월 3일에 입사했습니다.'로 출력하는 query을 작성
...
1. nvl(컬럼, 대체값) : 첫번째 인자값이 null이면 0으로 대체해서 출력한다.
   - 대체할 값이 숫자이면 두번재 인자값에 숫자를 지정한다.
   - 대체할 값이 문자이면 두번재 인자값에 문자를 지정한다.
   - 대체할 값이 날짜이면 두번재 인자값에 날짜를 지정한다.
---------------------------------*/
...
일반함수(NULL)
4. coalesce(컬럼1,컬럼2)
  - 컬럼1, 컬럼2 모두 Null이 아니면 컬럼 1을 리턴한다.
...
----------------------------------------*/
--department_ id가 10이면 'ACCOUNTING', 20이면 'RESEARCH',
...
--로 처리하고 사원명(first_name), 입사일(hire_date),분기로 출력
...
--salary 값이 10000미만이면  '하',10000이상~20000미만이면 '중',20000이상이면'상'
--으로 출력하도록 query문을 작성
SELECT first_name, salary,
...
--ORA-00937: not a single-group group function(단일 그룹의 그룹함수가 아닙니다.) 오류*
...
--50이하인 부서번호에서 대해서 NULL이 아닌 부서별의 직원수를 출력하시오.
--방법 1
SELECT department_id, count(*)
FROM employees
...
--부서별 최소급여, 최대급여를  최소값와 최대값이 같지않은 경우에만 부서별 오름차순으로 하세요.
...
2. ROWNUM은 조회도는 행수를 제한할 때 많이 사용한다.
...
2. SELECT문에서 rowid르 사용할 수 있다.
3. rowid을 통해서 데이터가 어떤 데이터파일, 어느 블록에 저장되어 있는지 알수있다.
3. rowid 구조 (총 18자리)
